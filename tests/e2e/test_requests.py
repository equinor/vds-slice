...
def test_assure_no_unauthorized_access(path, query, sas, error_msg):
    ...
    common_metadata = json.loads(multipart_data.parts[0].content)
    metadata = multipart_data.parts[1].content
    data = multipart_data.parts[2].content

    assert common_metadata['HasError'] == True
    assert len(common_metadata['Errors']) == 1
    assert error_msg in common_metadata['Errors'][0]

    assert len(metadata) == 0
    assert len(data) == 0


@pytest.mark.parametrize("path, query, error_code, errors", [
    (
        "nonexisting",
        {},
...
def test_errors_are_forwarded(path, query, error_code, errors):
    ...
    assert common_metadata['HasError'] == True
    assert len(common_metadata['Errors']) == len(errors)
    for i in range(0, len(errors)):
        assert errors[i] in common_metadata['Errors'][i]
...
def request_slice(method, lineno, direction):
    ...
    if method == "get":
        ...
        rdata = requests.post(f'{ENDPOINT}/slice', data=query)
        rdata.raise_for_status()
    else:
    ...
    assert len(multipart_data.parts) == 3
    common_metadata = json.loads(multipart_data.parts[0].content)
    metadata = json.loads(multipart_data.parts[1].content)
    data = multipart_data.parts[2].content

    assert common_metadata['HasError'] == False
    assert common_metadata['Errors'] == []
    assert common_metadata['Format'] == "bytearray"
